...
<head>
    ...
    <title>Stock Out - Coffee Shop Inventory</title>
...
<body class="bg-stone-100 min-h-screen text-gray-800 antialiased">
    ...
    <main class="lg:ml-64 p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto w-full">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 lg:mb-8">
            <div>
                ...
                <p class="text-sm text-stone-500 mt-1 tabular-nums">
                    {{ $stockOuts->count() }} {{ $stockOuts->count() == 1 ? 'record' : 'records' }} logged
                </p>
            </div>
        ...
        @endif
        <!-- Stock Out Table -->
        <section class="bg-white rounded-xl shadow-sm border border-stone-200/70 overflow-hidden" aria-label="Stock out list">
                ...
                <div class="flex items-center justify-between gap-3">
                    <div>
                        ...
                        <p class="text-xs sm:text-sm text-stone-500 mt-0.5">
                            Ingredients released from inventory
                        </p>
                    </div>
                </div>
            </div>
                    ...
                    <tbody class="divide-y divide-stone-100">
                        ...
                        <tr class="hover:bg-stone-50/70 transition">
                            ...
                            <td class="px-5 lg:px-6 py-4 min-w-36">
                                ...
                                <div class="text-xs sm:text-sm text-stone-500 tabular-nums">
                                    Current Stock:
                            ...
                            <td class="px-5 lg:px-6 py-4 text-right whitespace-nowrap">
                                <span class="font-semibold text-red-700 tabular-nums">-{{ number_format($stockOut->quantity, 2) }}</span>
                                <span class="inline-flex items-center bg-stone-100 text-stone-600 px-2 py-0.5 rounded-full text-xs font-medium whitespace-nowrap ml-1">{{ $stockOut->ingredient->unit ?? '' }}</span>
                            </td>
                            <!-- Date -->
                            ...
                            <td class="px-5 lg:px-6 py-4 text-stone-500 min-w-32">
                                @if($stockOut->notes)
                                <span class="block truncate max-w-48">{{ $stockOut->notes }}</span>
                                @else
                                <span class="text-stone-400">No notes</span>
                                @endif
                            </td>
                            <!-- Actions -->
                            <td class="px-5 lg:px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('stock-outs.edit', $stockOut) }}"
                                        aria-label="Edit stock out record {{ $stockOut->id }}"
                                    ...
                                    <form action="{{ route('stock-outs.destroy', $stockOut) }}"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            aria-label="Delete stock out record {{ $stockOut->id }}"
                                ...
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-stone-500 mt-1">
                                            Get started by recording your first release.
                                        </p>
                                    </div>
                                    <a
                                        href="{{ route('stock-outs.create') }}"
